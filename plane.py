...
class Enemy(Plane):
	...
	def __init__(self,speed):
		...
		self.image = pygame.image.load(planeImageName).convert_alpha()
		#敌人飞机原始位置
		self.x = random.randint(20,380)    #敌机出现的位置任意
		self.y = 0
		...
		for i in cls.g_ememyList:
			...
			for j in cls.hero.bulletList:
				...
				if enemyRect.colliderect(bulletRect):
					...
					if enemyRect.width == 76:
						cls.score += 1000     #小中大飞机分别100,500,1000分
					elif enemyRect.width == 101:
						cls.score += 5000
					elif enemyRect.width == 113:
						cls.score += 10000
...
if __name__ == '__main__':
    ...
    pygame.display.set_caption('Hack-running')
